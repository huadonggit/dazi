function game(){
  ...
  this.randarr=[];
  window.that=this;
  this.stepEle=document.getElementsByClassName('step')[0].getElementsByTagName('span')[1];
  this.lifeEle=document.getElementsByClassName('life')[0].getElementsByTagName('span')[1];
...
game.prototype={
  ...
  createEle:function(num){
    ...
    for(var i=0;i<arr.length;i++){
      ...
      div.style.cssText="width:150px;height:150px;position:absolute;left:"+(this.width-200)*Math.random()+50+"px;top:"+((-100)+(-100)*Math.random())+"px;color:rgba(0,0,0,0);font-size:60px;font-weight:bold;background-image:url(imgs/"+arr[i]+".png);background-size:150px 150px";
      div.innerHTML=arr[i];
